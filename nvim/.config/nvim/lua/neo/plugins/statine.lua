          ...
          lualine_x = {
            ...
            },
            {
              function()
                ...
                local wildcard_path = "/sys/class/leds/input*::capslock/brightness"
                local brightness_path = vim.fn.glob(wildcard_path)
                if not brightness_path or brightness_path == "" then
                  return ""
                end
                local file, err = io.open(brightness_path, "r")
                if not file then
                  return ""
                end

                local brightness_value = file:read("*a")
                file:close()

                local is_enabled = brightness_value:gsub("%s+", "") == "1"
