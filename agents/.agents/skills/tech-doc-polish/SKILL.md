...
4. **不夸大、不缩小**：程度词与事实相符；没有依据的强化词和淡化词都删。
5. **补充必要的引用参考来源**：数据、结论、他人观点应有出处；LLM 辅助撰写的文章尤其要检查引用是否缺失、是否真实。
6. **不用黑话**：有规范的技术术语或平实的表述方式时，不用行业黑话、圈内俚语。
7. **不过度口语化**：用平实的书面语陈述；聊天式的口语说法改为中性的书面表述。
8. **不高屋建瓴**：不写宏大叙事、行业趋势、时代背景式的空泛表达。
9. **不用浮夸词汇营造宏大框架感**：不用"底层逻辑""顶层设计""战略高度""生态位""范式""方法论"等看似宏大的术语，把具体技术问题包装成虚假完整的体系框架。
10. **不用浮夸词**：不用"关键洞察"这类自我抬升身价的标签词，也不用"一句话总结："这类元叙述式的自我存在感表述。
