...
16. **意图在前，做法在后；遵循因果顺序**：先说明原因、目的或概念，再给出操作、细节、结果或决策；段落和句子按合理的因果顺序展开，不要让读者在尚未理解“为什么”和相关背景时先看到做法或抉择。
...
需要区分的是，并不是所有「我」都要删除。作者的真实选择、实践经历或决策理由若对读者理解技术内容有价值，可以保留；只删去情绪性、评价性或元叙述性的「我」。
...
- 改前：「选择 Redis 而不是 Memcached，因为 Redis 支持持久化。」（读者尚未了解两者的差异或需求背景）
