...
3. **客观陈述**：只陈述事实、数据、逻辑，删去作者的情绪和评价姿态。非必要不主动写「是 xxx，不是 xxx」「不是 xxx，而是 xxx」这类防御式或讨论式句式；先判断读者是否真的会误解，不要把作者内心的辩驳痕迹留给读者看。
...
7. **引用链接信息完整**：如果文档包含参考小节，每条引用链接应同时给出：1) 链接的字面文本；2) 链接本身；3) 对该引用项的简要说明。
8. **不用黑话**：有规范的技术术语或平实的表述方式时，不用行业黑话、圈内俚语。
9. **不过度口语化**：用平实的书面语陈述；聊天式的口语说法改为中性的书面表述。
10. **不高屋建瓴**：不写宏大叙事、行业趋势、时代背景式的空泛表达。
11. **不用浮夸词**：不用"关键洞察""深度好文""重磅""干货""必看""一文读懂""保姆级"等自我抬升身价的标签词；不用"底层逻辑""顶层设计""战略高度""生态位""范式""方法论"等营造宏大框架感的术语；也不用"一句话总结："这类元叙述式的自我存在感表述。
12. **不戏剧化**：不制造悬念、反转、冲突感。
13. **转折有铺垫**：不用突然的"然而""但是"制造意外；先陈述对照的事实，再转折。
14. **避免元叙述**：不谈论"我正在写什么/如何写/读者应如何读"，而是直接切入话题本身。作者的真实选择、实践经历或决策理由若对读者理解技术内容有帮助，可以保留；只删去情绪性、评价性或元叙述性的“我”。
15. **减少重复性表述**：避免同一句式、同一词语在相邻段落中反复出现；合并意思相近的句子，保持语言简洁流畅。
16. **意图在前，做法在后；遵循因果顺序**：先说明原因、目的或概念，再给出操作、细节、结果或决策；段落和句子按合理的因果顺序展开，不要让读者在尚未理解“为什么”和相关背景时先看到做法与决策。
...
- 改前：「这个工具是代码生成器，不是编译器。」（读者并未混淆两者）
- 改后：「这个工具根据模板生成代码。」
- 改前：「这里用的是乐观锁，不是悲观锁。」（没有说明为什么相关）
- 改后：先说明业务场景再给出理由：「该场景读多写少，冲突概率低，因此使用乐观锁。」
- 改前：「这不是缓存失效，而是并发写入导致的竞态。」（读者尚未看到现象描述）
- 改后：先描述现象再给出结论：「请求 A 和请求 B 几乎同时写入同一条记录，后一次写入覆盖了前一次；因此这不是缓存失效，而是并发竞态。」
- 识别信号：感叹号、感情色彩形容词（惊人、惊艳、超赞）、第一人称情绪表达；预先的「不是」「并非」「区别于」「注意，这里不是」等防御式否定，且被否定的对象并非读者真正关心的误解点。
...
- 如果文档设有「参考」「参考资料」「References」等小节，每条引用链接应信息完整：
  1. **链接字面文本**：引用的标题或名称，如《Rust 程序设计语言》、GNU Stow 官方文档。
  2. **链接地址**：可点击或复制的 URL。
  3. **简要说明**：一句话交代该引用项的内容或用途，例如「官方安装指南」或「介绍 stow 符号链接机制的章节」。
  ...
  - 改后：
    ```
    - [link](https://example.com/spec) —— 该规范定义了协议的消息格式与状态码。
    ```
...
元叙述是指作者不直接陈述技术内容本身，而是把「我正在写什么」「如何写」「读者应该如何读」这类自我指涉的说明插入正文。它会把作者的存在感、写作过程或文章结构挡在话题前面，打断读者对内容本身的关注。

并不是所有「我」都要删除。作者的真实选择、实践经历或决策理由若对读者理解技术内容有价值，可以保留；只删去情绪性、评价性或元叙述性的「我」。

- **可保留**：真实经验、具体决策、个人选择，且这些信息对读者理解技术内容有价值。
  - 「我最终选择了 SQLite，因为部署环境没有持久化存储。」——真实决策及其理由，读者可以参考。
  - 「我在排查时发现，问题出在 DNS 缓存上。」——具体实践经历，有助于定位类似问题。
- **应删去或改写**：情绪表达、评价姿态、元叙述、无信息量的自我存在感。
  - 「我觉得这个方案太优雅了。」→「这个方案把状态转换集中在单一模块里，减少了副作用。」（改为具体优点）
  - 「我将带你一步步了解。」→ 直接开始介绍。（删除元叙述式的引导）
  - 「我认为显而易见的是……」→ 直接陈述理由。

判断标准：去掉「我」之后，句子是否仍然传递了同等价值的技术信息。如果保留「我」能让读者获得真实决策背景或实践经验，则保留；否则改为客观陈述。
...
注意：必要的前置说明（如「本文假设读者已了解 X」）和适度的承上启下（如「回到 HTTP 映射」）不算是元叙述；应清理的是把作者姿态、写作过程或文章结构本身当作内容的表述。
...
- 改前：「执行 `git pull`，这样能保持仓库状态最新。」
- 改后：「为了保持仓库状态最新，先执行 `git pull`。」
- 改前：「运行 `stow -S nvim`，它会把 Neovim 配置同步到 home 目录。」
- 改后：「要把 Neovim 配置同步到 home 目录，运行 `stow -S nvim`。」
- 改前：「`lazy-lock.json` 记录了插件的固定版本。它是 lazy.nvim 的 lock 文件。」
- 改后：「lazy.nvim 用 `lazy-lock.json` 记录插件的固定版本。」
- 改前：「选择 Redis 而不是 Memcached，因为 Redis 支持持久化。」（读者尚未了解两者差异和需求背景）
- 改后：先说明场景需求（需要缓存且部分数据重启后仍需保留），再给出决策：「因此选择 Redis 作为缓存，而不是仅提供内存存储的 Memcached。」
- 识别信号：做法在前、目的在后；细节在前、概念在后；结果在前、原因在后；决策在前、背景在后；段落顺序不符合“因为→所以”“目的→手段”“概念→细节”“背景→决策”的因果链。
